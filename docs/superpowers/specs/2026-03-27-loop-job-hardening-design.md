...
保留不变的参数：`--dangerously-skip-permissions`、`--max-turns 50`、`--verbose`、`--output-format stream-json`（使用 `stream-json` 而非 `text`，支持实时逐行输出到日志，避免任务完成前无法监控进度；`stream-json` 要求 `--verbose`）。
