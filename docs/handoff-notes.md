...
macOS runtime validation under the new `Wink` identity is now partially complete: Accessibility/TCC recovery, standard-route packaged-app E2E, and Settings drag-to-reorder persistence were revalidated on 2026-04-20 against the current standard-only fixture. Hyper/event-tap under the new identity and launch-at-login still remain pending because `com.wink.app` is a new TCC/login-item identity.
