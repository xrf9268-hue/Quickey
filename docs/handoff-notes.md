...
## Current State
On 2026-04-18, issue #176 added the first pass of Hotkey Recipes to the codebase: Quickey now has a separate versioned `.quickeyrecipe` JSON schema plus codec, deterministic import planning that resolves apps by Bundle ID then app name, preview-first conflict handling (`Skip Conflicts` / `Replace Existing`), and truthful unresolved-app persistence instead of dropping unmatched entries. The new UI work lives in the Settings `Shortcuts` tab as `Export...` / `Import...` actions with an inline import preview card. This was implemented from Linux by code inspection and unit-test updates only; real macOS validation is still required for `NSOpenPanel` / `NSSavePanel`, inline preview UX, and the unresolved-row presentation in the live Settings window.
