...
struct ShortcutsTabView: View {
    ...
    @ViewBuilder
    private func importPreviewCard(_ plan: QuickeyRecipeImportPlanner.ImportPlan) -> some View {
        CardView("Import Preview") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !plan.unresolvedEntries.isEmpty {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Unresolved apps will still be imported using their recipe app name and Bundle ID.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
